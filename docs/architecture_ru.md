...
* ru_RU.json - перевод основных строк на русский + строки, которые не используются мной непосредственно, но которые попадают в перевочик. Последние оставил чтобы они не забивали переводчик базы
...
** combo - комбобокс, помимо имени и типа содержит еще поле query описывающее какой запрос нужно задействовать для получения списка значений в подстановку.
