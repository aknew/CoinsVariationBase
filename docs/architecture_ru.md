...
*name - название БД, которое будет отображаться в различных местах. В данный момент не используется 
*startTable - имя ноды с которой будет начато отображение
*nodes - массив нод, структуру элемента подробнее смотри ниже
*comboBoxes - массив элементов на основании которых берутся комбобоксов, каждый элемент состоит из sql-запроса в базу и идентификатора (используется в описании интерфейса)
...
*subNodes - массив сабнот, полностью идентичен по описанию childNode, отличается только отображение
*fullFormFields - список полей, отображаемых в полном виде записи. Структура элемента - имя поля из таблицы, тип поля (в данный момент их всего три - просто текст, екомбобокс и картинка, причем картинка требует рефаторинга)
...
*импорт в различные форматы только для чтения (pdf, форматы электронных книг, возможно, статический html)
*парсинг некоторых сайтов (есть пробный по coins.su, в репозитории пока отсутствует)