...
* Подчиненая таблица, например, ссылки на источники, не показываются отдельно от своей главной таблицы. Используются основной таблицей для получения списка и для внесения изменений. В данный момент не реализована и не уверен что будет
...
**CBImageProvider** - провайдер изображений, наследник QQuickImageProvider. Имеет тесную инетграцию с CBAttachmentsProvider, что необходимо чтобы знать откуда показывать картинки, если вложение не является картикнкой будет показана иконка файла
...
* LabeledTextInput.qml - текст + заглавие, выполнено на основании Text - так было проще

* LabeledComboBoxInput.qml - текстовое поле с выбором, изначально хотел использовать ComboBox из QtQuick.Controls, но оказалось что там жестко забит пробел как кнопка вызова списка и использовать его при наборе не удается
...
* ru_RU.json - перевод основных строк на русский + строки, которые не используются мной непосредственно, но которые попадают в перевочик. Посление оставил чтобы они не забивали переводчик базы
...
**просто текст - тип не указывается, поле будет выведено при помощи простого текстового поля с надписью
**combo - комбобокс, помимо имени и типа содержит еще поле query описывающее какой запрос нужно задействовать для получения списка значений в подстановку. Для отображения списка задействован самописный диалог т.к. в родном комбобоксе на пробел повешено скрытие списка и использовать его как часть поля не удается
**date - дата, выводится календарь для выбора данных, отображается как текстовое поле с ИСО датой
**hyperlink - при двойном щелчке вохможен переход по ссылке, не реализовано
